    ...
    try {
        ...
        json.recordset.map((rows)=>{
            if(rows.CODCATEGORIA==1){
                str1 += `<option value=${rows.CODPROD}>${rows.DESCRIPCION}</option>`
            };
            ...
            if(rows.CODCATEGORIA==3){
                str3 += `<div class="form-group">
                                    <input type="checkbox" checked data-toggle="toggle" data-on="SI" data-off="NO" data-onstyle="success" data-offstyle="danger" id='${rows.CODPROD}'>
                                    <label>${rows.DESCRIPCION}</label>
                                    </div>`
            };
    ...
    funciones.Confirmacion('¿Está seguro que desea Guardar esta Orden?')
        .then(async (value)=>{
            if(value==true){
                ...
                let categoria1 = document.getElementById('contenedor1').value;
